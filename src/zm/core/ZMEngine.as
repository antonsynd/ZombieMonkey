...
package zm.core
{
	...
	public final class ZMEngine extends EventDispatcher
	{
		...
		public function eval(script:String, context:String = null):String
		{
			...
			var hashCode:String = generateHashCode();
			var bytes:ByteArray;
			bytes = _compileStringToBytes("try { with ('zm.core'::ZMEngine.engine.getGlobal('" + hashCode + "')) { " + directives + script + " } } catch (error) { 'zm.core'::ZMEngine.engine.handleError('" + hashCode + "', error); } 'zm.core'::ZMEngine.engine.updateGlobal('" + hashCode + "', this); 'zm.core'::ZMEngine.engine.kill('" + hashCode + "');", context ? context : hashCode);
			bytes.position = 0;
